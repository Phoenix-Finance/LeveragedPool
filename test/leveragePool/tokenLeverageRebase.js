...
contract('leveragedPool', function (accounts){
    ...
    it('leveragedPool normal tests', async function (){
        await testTokenRebase(beforeInfo.USDC,beforeInfo.WBTC,beforeInfo,accounts);
        return;
        await testTokenRebase(beforeInfo.USDC,beforeInfo.WETH,beforeInfo,accounts);
    });
    it('leveragedPool normal tests 2', async function (){
        return;
        await testTokenRebase2(beforeInfo.USDC,beforeInfo.WBTC,beforeInfo,accounts);
        await testTokenRebase2(beforeInfo.USDC,beforeInfo.WETH,beforeInfo,accounts);
    });
    async function logInfo(tokenA,tokenB,contracts){
        ...
        netWroth = await contracts.leveragePool.getCurrentLeverageRate();
        console.log("current leverage rate : ",netWroth[0].toString(),netWroth[1].toString());
        console.log("===============================================================");
    }
    async function testTokenRebase(tokenA,tokenB,beforeInfo,accounts){
...
//        console.log(events);
        ...
        await logInfo(tokenA,tokenB,contracts);
        await leverageCheck.buyLeverage(eventDecoder,contracts,1,tokenAmount1,0,accounts[0]); 
        let priceStep = priceB.divn(50)
        for (var i=1;i<21;i++){
            prices = [priceA,priceB.sub(priceStep.muln(i))]
            await testInfo.setOraclePrice(assets,prices,factoryInfo,beforeInfo.pair,accounts[0]); 
            await logInfo(tokenA,tokenB,contracts);
            await factoryInfo.factory.rebalanceAll();
            await logInfo(tokenA,tokenB,contracts);
        ...
        for (var i=1;i<21;i++){
            prices = [priceA,priceB.add(priceStep.muln(i))]
            await testInfo.setOraclePrice(assets,prices,factoryInfo,beforeInfo.pair,accounts[0]); 
            await logInfo(tokenA,tokenB,contracts);
            await factoryInfo.factory.rebalanceAll();
            await logInfo(tokenA,tokenB,contracts);
