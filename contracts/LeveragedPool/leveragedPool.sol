...
contract leveragedPool is leveragedData{
    ...
    function setLeverageFee(uint256 _buyFee,uint256 _sellFee,uint256 _rebalanceFee) onlyOperator2(0,1) public{
        buyFee = _buyFee;
        sellFee = _sellFee;
    ...
    function _buy(leverageInfo memory coinInfo,uint256 amount,uint256 minAmount,bytes memory /*data*/) getUnderlyingPrice internal{
        ...
        if(coinInfo.id == 0){
            emit BuyLeverage(msg.sender,coinInfo.token,amount,leverageAmount);
        }else{
            emit BuyHedge(msg.sender,coinInfo.token,amount,leverageAmount);
        }     
    }
    ...
    function _sell(leverageInfo memory coinInfo,uint256 amount,uint256 minAmount,bytes memory /*data*/) getUnderlyingPrice internal{
        ...
        if(coinInfo.id == 0){
            ...
            emit SellHedge(msg.sender,amount,repay);
        }
    }
    ...
    function rebalance() getUnderlyingPrice onlyOperator2(0,1) public {
        _rebalance();
    }
